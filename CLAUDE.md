...
### Phase 18 — 실사용 불만 개선 A: 필터 정밀화 (완료, 2026-07-18)
> 불만: ① 무관 분야 알림 다수 ② 메일 과다 ③ 가독성 — 이번엔 ①만 (A)
> 진단: 오탐 3건 전부 "인공지능" 단독 키워드가 원인 (2건은 본문 매칭)
- [x] **본문(description) 단독 매칭 제거** — 통과 판정은 제목+분야만
  - `FilterEngine._primary_text()` 신설, matches()가 primary 기준으로 판정
  - "인공지능집적단지 연구"가 본문에 스쳐 통과하던 오탐 차단
- [x] **조건부 키워드(conditional_keywords) 신설** — 인공지능·AI
  - 단독 통과 불가, 핵심 키워드와 동시 등장 시에만 통과 (심리×AI 융합만)
  - "인공지능혁신인재양성" 차단 / "인공지능 기반 심리상담" 통과
- [x] keywords 보강: 노인·인문(인문분야 포괄)·뇌과학·신경과학·상담·정서·정신의학
  - "학습"(기계학습 오탐)·"중견연구"(전 분야 공통) 등은 의도적 제외
- [x] 검증: 회귀 19/19, 배포 111건 오탐 0건, 인문사회 재현율 156/156(100%)
  - 심리학 라벨 재현율 71%: 미통과분은 전 분야 공통 기초연구사업(중견·신진) — 정밀도 우선 의도적 제외

### Phase 18-B+C — 일일 다이제스트 + 카드형 메일 (완료, 2026-07-18)
- [x] **발송 구조 전환**: 매 6시간 즉시 발송 → 매일 09:00 KST 이후 첫 사이클에서 1통 통합
  - `run_daily_digest()` 신설 (scheduler.py) — 신규 + 리마인더 한 통으로
  - `DigestLog` 테이블: KST 날짜당 최대 1통 보장, 내용 없으면 침묵(count=0 기록)
  - 발송 실패 시 미기록 → 같은 날 다음 사이클(15시)이 자동 재시도
  - Actions cron 변경 불필요 (00:00 UTC = 09:00 KST 사이클이 담당)
- [x] **리마인더 확장**: 신규 확인 1회 + D-30/D-14/D-7/D-3/D-1 (기존 d7/d3/d1)
  - REMINDER_THRESHOLDS 오름차순 재정렬 — 가장 긴급한 미발송 단계 하나만 선택
  - 당일 신규로 소개된 공고는 리마인더 중복 제외
- [x] **카드형 메일(C)**: 공고당 카드 — 출처·D-day 색상 배지, 매칭 근거 칩("왜 왔는지"),
  접수기간, 분야, 예산, 공고 보기 버튼. 모바일 1열(600px), 인라인 스타일
  - `FilterEngine.match_reasons()` 신설 — 매칭 근거 추출 (조건부는 "인공지능×심리" 표기)
- [x] 검증: 로직 테스트 10/10 (분류·단계 선택·1일 1통·침묵·멱등) + 미리보기 실발송
- 참고: `run_reminder_cycle`은 레거시로 유지 (test_reminder.py용, 운영 미사용)

### Phase 19 — 동작 안정성 3종: 고장 감지·수집 확대·마감 갱신 (완료, 2026-07-18)
- [x] **O1 침묵 고장 감지** — "고장난 줄 모르는 고장"(NRF 한 달 침묵 사고) 재발 방지
  - BaseScraper.health: 파싱 원시 개수 기준 구조 이상 판정 (접수중 0건은 정상 취급)
  - `SourceHealth` 테이블: 연속 실패 카운터 — 2회(12시간) 이상만 경보 (일시 오류 무시)
  - 다이제스트 발송 실패도 경보 대상
  - `data/health_fail.flag` → scrape.yml 마지막 "수집 상태 점검" 단계가 워크플로 실패 처리
    → GitHub 실패 알림 메일 발송 (데이터 커밋 후 실행이라 수집분은 보존)
- [x] **O2 수집 범위 확대** — 유실 방지
  - NTIS: pageUnit=100 (기존 10건/사이클 → 100건). 검증: 100건 수집
  - IRIS: pageIndex 폼 조작으로 전체 페이지 순회 (기존 1페이지 10건 → 전체). 검증: 25/25건
- [x] **O3 마감일 연장 반영**
  - `_save_new_items`: 기존 URL 공고의 마감일 변경 시 갱신 + 리마인더 발송 기록 삭제
    → 새 마감일 기준 D-30~D-1 단계 재발송. category 빈 값 보완도 함께
  - 검증: 갱신·기록 삭제·신규 미중복 단위 테스트 통과
- [x] 다이제스트 회귀 10/10 재확인

### Phase 17-b — 경미 항목 정리 (완료, 2026-07-07)
- [x] **UTC/KST 시차 수정** — `src/config.py`에 `today_kst()` 추가
  - Actions가 UTC라 D-day가 최대 9시간 이르게 계산되던 문제
  - 적용: scheduler(리마인더), email(D-day 표시), nrf(접수중 판정), analytics(준비 시작일), web
- [x] **sync notice_type 오기록 수정** — 마감일 기준으로 접수중/접수마감 판별
- [x] **캐시 유실 시 중복 알림 방지** — `src/export.py` `restore_announcements()`
  - 커밋된 announcements.json에서 운영 DB 복원 (is_notified=True + 알림 이력까지)
  - run_scrape 시작 시 자동 실행. 검증: 빈 DB에서 93건+이력 108건 복원, 멱등 확인
  - 한계: 제목 중복 병합으로 JSON에서 빠진 공고는 1회 재알림 허용
...
| 2 | ~~이메일 알림 실패 원인 확인~~ | ⭐ | ✅ 해결 (Secret 설정 후 발송 정상, 2026-07 기준 성공 88건) |
| 3 | ~~과거 데이터 패턴 분석~~ | ⭐⭐⭐ | ✅ 완료 (Phase 13~16, `docs/analytics.html`) |
| 4 | **주간 "준비 알림" 메일** | ⭐⭐ | Preparation Calendar의 action_required를 주 1회 메일로 발송 |
| 5 | 정밀 라벨링 | ⭐⭐ | 사용 경험 기반 키워드/카테고리 조정 (추후) |
